...
export function CampusMapsPage() {
  ...
  const filteredMaps = useMemo(
    () =>
      data.maps.filter((map) => {
        const keywordMatch =
          !query ||
          [map.name, map.description, map.creator, map.fileName]
            .join(" ")
            .toLowerCase()
  ...
  const selectedMap =
    data.maps.find((map) => map.id === selectedMapId) ??
    filteredMaps[0] ??
    data.maps[0] ??
    null;
  const currentBedPoints = selectedMap
    ? data.bedPoints.filter((point) => point.mapId === selectedMap.id)
    : [];
  const currentNavPoints = selectedMap
    ? data.navigationPoints.filter((point) => point.mapId === selectedMap.id)
    : [];

  useEffect(() => {
    const locate = readState<LocateState>(CAMPUS_LOCATE_KEY);
    if (locate) {
      setSelectedMapId(locate.mapId);
      setHighlightPointId(locate.pointId);
      setZoom(1.35);
      setPan({ x: 0, y: 0 });
      setMessage("已从病床列表定位到地图点位。");
  ...
  const submitMap = () => {
    ...
    updateCampus((next) => {
      if (editingMap) {
        ...
        appendLog(next, "编辑地图", `编辑地图 ${mapDraft.name}。`);
      } else {
        const created: CampusMap = {
        ...
        next.maps.unshift(created);
        appendLog(next, "新增地图", `新增${created.type} ${created.name}。`);
  ...
  const confirmDeleteMap = () => {
    ...
    updateCampus((next) => {
      ...
      appendLog(next, "删除地图", `删除地图 ${deleteMap.name}。`);
    });
    setDeleteMap(null);
  };
      ...
      if (editingBedPoint) {
        ...
        next.bedPoints = next.bedPoints.map((point) =>
          point.id === pointId
            ? {
                ...point,
                x: clampPercent(bedPointDraft.x),
                y: clampPercent(bedPointDraft.y),
                note: bedPointDraft.note,
                updatedAt: now
              }
            : point
        );
        next.beds = next.beds.map((bed) =>
          bed.id === editingBedPoint.bedId
            ? {
                ...bed,
                code: bedPointDraft.code.trim(),
                name: bedPointDraft.name.trim(),
                wardName: bedPointDraft.wardName.trim() || selectedMap.name,
                type: bedPointDraft.type,
                status: bedPointDraft.status,
                note: bedPointDraft.note,
        ...
        appendLog(next, "编辑点位", `编辑病床点位 ${bedPointDraft.code}。`);
      } else {
        ...
        appendLog(next, "新增点位", `新增病床点位 ${bedPointDraft.code} 并自动创建病床。`);
      }
    });
  ...
  const deleteBedPoint = (point: BedPoint) => {
    updateCampus((next) => {
      const bed = next.beds.find((item) => item.id === point.bedId);
      ...
      appendLog(next, "删除点位", `解除病床 ${bed?.code ?? point.bedId} 的地图点位关联。`);
    });
  };
  ...
  const submitNavPoint = () => {
    ...
    if (!navPointDraft.name.trim() || !navPointDraft.code.trim()) {
      return;
    }
    ...
    updateCampus((next) => {
      ...
      if (editingNavPoint) {
        next.navigationPoints = next.navigationPoints.map((point) =>
          point.id === editingNavPoint.id
            ? {
                ...point,
                name: navPointDraft.name.trim(),
                code: navPointDraft.code.trim(),
                type: navPointDraft.type,
                description: navPointDraft.description,
                note: navPointDraft.note,
                x: clampPercent(navPointDraft.x),
                y: clampPercent(navPointDraft.y),
                updatedAt: now
              }
            : point
        );
        appendLog(next, "编辑点位", `编辑导航点位 ${navPointDraft.code}。`);
      } else {
        next.navigationPoints.unshift({
          id: generateId("nav-point"),
          mapId: selectedMap.id,
          name: navPointDraft.name.trim(),
          code: navPointDraft.code.trim(),
          type: navPointDraft.type,
        ...
        appendLog(next, "新增点位", `新增导航点位 ${navPointDraft.code}。`);
      }
    });
  ...
  const deleteNavPoint = (point: NavigationPoint) => {
    updateCampus((next) => {
      ...
      appendLog(next, "删除点位", `删除导航点位 ${point.code}。`);
    });
  };

  const locateBed = (bedId: string, pointId: string) => {
    writeState(CAMPUS_FOCUSED_BED_KEY, { bedId });
    setPointPreview(null);
    navigate({ to: "/campus/beds" });
    setHighlightPointId(pointId);
  };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
            ...
            <TableBody>
              {filteredMaps.map((map, index) => (
                <TableRow
                  ...
                  data-state={selectedMap?.id === map.id ? "selected" : undefined}
      ...
      <Card className={cn("overflow-hidden", fullscreen && "fixed inset-4 z-50 flex flex-col")}>
        <CardHeader className="border-b border-border/60">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <CardTitle>{selectedMap?.name ?? "地图详情"}</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                {selectedMap ? `${selectedMap.type} · ${selectedMap.fileName}` : "请选择地图"}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => setZoom((value) => Math.min(2, value + 0.1))}>
                <ZoomIn className="h-4 w-4" />
                放大
              </Button>
              <Button variant="outline" size="sm" onClick={() => setZoom((value) => Math.max(0.65, value - 0.1))}>
        ...
        <CardContent className="grid min-h-[520px] flex-1 gap-4 p-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div
            ref={mapAreaRef}
            className="relative min-h-[520px] overflow-hidden rounded-[1.5rem] border border-border/70 bg-surface-100"
          ...
          >
            <div
              className="absolute inset-8 rounded-[1.75rem] border border-surface-300 bg-white shadow-inner"
                ...
                ? currentBedPoints.map((point) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className={cn(
                          "absolute flex h-16 w-20 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-2xl border-2 border-white text-xs font-semibold shadow-panel transition hover:scale-105",
          ...
          <div className="space-y-3">
            <SectionCard
              title={selectedMap?.type === "病房地图" ? "病床点位" : "导航点位"}
              description={selectedMap?.type === "病房地图" ? "病床点位状态颜色与病床使用状态同步。" : "导航地图仅维护机器人导航点位。"}
            >
            ...
            <SectionCard title="业务规则">
              <div className="space-y-2 text-sm leading-7 text-muted-foreground">
                <p>病床点位仅允许在病房地图中创建，导航地图仅允许维护机器人导航点位。</p>
                <p>删除病床点位只解除地图关联关系，不删除病床数据。</p>
                <p>删除地图前会校验病床点位和导航点位关联。</p>
              </div>
            </SectionCard>
          </div>
        </CardContent>
      ...
      <DialogFormShell
        open={bedPointDialogOpen}
        onOpenChange={setBedPointDialogOpen}
        title={editingBedPoint ? "编辑病床点位" : "新增病床点位"}
        description="保存后自动同步病床管理；删除点位只解除地图关联。"
        onSubmit={submitBedPoint}
        submitLabel="保存"
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="病床编号" required>
            <Input value={bedPointDraft.code} onChange={(event) => setBedPointDraft((current) => ({ ...current, code: event.target.value }))} />
          </Field>
          <Field label="病床名称" required>
            <Input value={bedPointDraft.name} onChange={(event) => setBedPointDraft((current) => ({ ...current, name: event.target.value }))} />
          </Field>
          <Field label="所属病房">
            <Input value={bedPointDraft.wardName} onChange={(event) => setBedPointDraft((current) => ({ ...current, wardName: event.target.value }))} />
          </Field>
          <Field label="病床类型">
            <Input value={bedPointDraft.type} onChange={(event) => setBedPointDraft((current) => ({ ...current, type: event.target.value }))} />
          </Field>
          <Field label="病床状态">
          ...
          <div className="md:col-span-2">
            <Field label="备注">
              <Textarea value={bedPointDraft.note} onChange={(event) => setBedPointDraft((current) => ({ ...current, note: event.target.value }))} />
            </Field>
          </div>
        </div>
      ...
      <DialogFormShell
        open={navPointDialogOpen}
        onOpenChange={setNavPointDialogOpen}
        title={editingNavPoint ? "编辑导航点位" : "新增导航点位"}
        description="导航地图不允许创建病床点位，仅用于机器人导航。"
        onSubmit={submitNavPoint}
        submitLabel="保存"
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="点位名称" required>
            <Input value={navPointDraft.name} onChange={(event) => setNavPointDraft((current) => ({ ...current, name: event.target.value }))} />
          </Field>
          <Field label="点位编码" required>
            <Input value={navPointDraft.code} onChange={(event) => setNavPointDraft((current) => ({ ...current, code: event.target.value }))} />
          </Field>
          <Field label="点位类型">
          ...
          <div className="md:col-span-2">
            <Field label="描述">
              <Textarea value={navPointDraft.description} onChange={(event) => setNavPointDraft((current) => ({ ...current, description: event.target.value }))} />
            </Field>
          </div>
          <div className="md:col-span-2">
            <Field label="备注">
              <Textarea value={navPointDraft.note} onChange={(event) => setNavPointDraft((current) => ({ ...current, note: event.target.value }))} />
            </Field>
          </div>
        </div>
      </DialogFormShell>
        ...
        <DialogContent>
          ...
          {selectedBedPoint && selectedBed ? (
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">病床编号</p>
                  <p className="mt-1 font-medium">{selectedBed.code}</p>
                </div>
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">病床名称</p>
                  <p className="mt-1 font-medium">{selectedBed.name}</p>
                </div>
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">当前状态</p>
                  <p className="mt-1 font-medium">{selectedBed.status}</p>
                </div>
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">当前使用状态</p>
                  <p className="mt-1 font-medium">{getBedDisplayStatus(selectedBed, data.usageRecords)}</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              {[
                ["点位名称", selectedNavPoint.name],
                ["点位编码", selectedNavPoint.code],
                ["点位类型", selectedNavPoint.type],
                ["描述信息", selectedNavPoint.description || "暂无描述"]
              ].map(([label, value]) => (
                <div key={label} className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
          ...
          {selectedBedPoint && selectedBed ? (
            <DialogFooter>
              ...
              <Button onClick={() => locateBed(selectedBed.id, selectedBedPoint.id)}>跳转病床详情</Button>
            </DialogFooter>
          ) : null}
        </DialogContent>
      </Dialog>
...
export function CampusBedsPage() {
  ...
  useEffect(() => {
    const focused = readState<{ bedId: string }>(CAMPUS_FOCUSED_BED_KEY);
    if (focused) {
      setSelectedBedId(focused.bedId);
      setMessage("已从地图点位跳转到病床详情。");
      clearState(CAMPUS_FOCUSED_BED_KEY);
    }
  }, []);
  ...
  const bedsWithUsage = useMemo(
    () =>
      data.beds.map((bed) => ({
        ...bed,
        usageStatus: deriveBedUsageStatus(bed, data.usageRecords),
        displayStatus: getBedDisplayStatus(bed, data.usageRecords),
        currentUsage: getCurrentUsage(bed, data.usageRecords),
        nextReservation: getNextReservation(bed, data.usageRecords)
      })),
  ...
  const selectedBed =
    data.beds.find((bed) => bed.id === selectedBedId) ??
    (selectedBedId ? null : data.beds[0] ?? null);
      ...
      if (editingBed) {
        ...
        if (bedDraft.mapId) {
          if (!pointId) {
            ...
            next.bedPoints.unshift({
              id: pointId,
              mapId: bedDraft.mapId,
              bedId: editingBed.id,
              x: clampPercent(bedDraft.x),
              y: clampPercent(bedDraft.y),
              note: bedDraft.note,
              createdAt: now,
              updatedAt: now
            });
        ...
        next.beds = next.beds.map((bed) =>
          bed.id === editingBed.id
            ? {
                ...bed,
                code: bedDraft.code.trim(),
                name: bedDraft.name.trim(),
                wardName: bedDraft.wardName.trim(),
                type: bedDraft.type,
                status: bedDraft.status,
                note: bedDraft.note,
                mapId: bedDraft.mapId || undefined,
                pointId,
                x: bedDraft.mapId ? clampPercent(bedDraft.x) : undefined,
                y: bedDraft.mapId ? clampPercent(bedDraft.y) : undefined,
                updatedAt: now
              }
            : bed
        );
        appendLog(next, "编辑病床", `编辑病床 ${bedDraft.code}。`);
        if (bedDraft.mapId) {
          appendLog(next, "病床绑定地图", `病床 ${bedDraft.code} 绑定地图 ${targetMap?.name ?? bedDraft.mapId}。`);
        }
      } else {
        const bedId = generateId("bed");
        ...
        appendLog(next, "新增病床", `新增病床 ${bedDraft.code}。`);
        if (bedDraft.mapId) {
          appendLog(next, "病床绑定地图", `病床 ${bedDraft.code} 绑定地图 ${targetMap?.name ?? bedDraft.mapId}。`);
        }
      }
  ...
  const confirmDeleteBed = () => {
    ...
    updateCampus((next) => {
      if (hasRecords) {
        next.beds = next.beds.map((bed) =>
          bed.id === deleteBed.id ? { ...bed, status: "停用", updatedAt: new Date().toISOString() } : bed
        );
        appendLog(next, "删除病床", `病床 ${deleteBed.code} 存在历史使用记录，已转为停用。`);
      } else {
        next.beds = next.beds.filter((bed) => bed.id !== deleteBed.id);
        next.bedPoints = next.bedPoints.filter((point) => point.bedId !== deleteBed.id);
        appendLog(next, "删除病床", `物理删除病床 ${deleteBed.code}。`);
      }
    });
    setDeleteBed(null);
  };
  ...
  const locateBedOnMap = (bed: BedRecord) => {
    ...
    writeState(CAMPUS_LOCATE_KEY, {
      mapId: bed.mapId,
      pointId: bed.pointId
    });
    navigate({ to: "/campus/maps" });
  };
  ...
  const submitUsage = () => {
    const bed = data.beds.find((item) => item.id === selectedBedId);
    if (!bed || !usageDraft.patientName.trim()) {
      return;
    }

    updateCampus((next) => {
      next.usageRecords.unshift({
        id: generateId("usage"),
        bedId: bed.id,
        patientName: usageDraft.patientName.trim(),
        startAt: usageDraft.startAt,
        expectedEndAt: usageDraft.expectedEndAt,
        registrar: "李琴",
        registeredAt: new Date().toISOString(),
        note: usageDraft.note
      });
      appendLog(next, "病床使用登记", `登记病床 ${bed.code} 给患者 ${usageDraft.patientName.trim()} 使用。`);
    });
  ...
  const submitEndUsage = () => {
    ...
    updateCampus((next) => {
      next.usageRecords = next.usageRecords.map((record) =>
        record.id === active.id
          ? {
              ...record,
              endAt: endDraft.endAt,
              endNote: endDraft.note
            }
          : record
      );
      next.beds = next.beds.map((bed) =>
        bed.id === endingBed.id
          ? {
              ...bed,
              lastEndedAt: endDraft.endAt,
              updatedAt: new Date().toISOString()
            }
          : bed
      );
      appendLog(next, "病床结束使用", `结束病床 ${endingBed.code} 的当前使用。`);
    });
    setEndingBed(null);
  };
  ...
  const selectedRecords = recordsBed
    ? data.usageRecords
        .filter((record) => {
          if (record.bedId !== recordsBed.id) {
            return false;
          }
          const fromMatch = !recordDateFrom || new Date(record.startAt) >= new Date(`${recordDateFrom}T00:00`);
          const toMatch = !recordDateTo || new Date(record.startAt) <= new Date(`${recordDateTo}T23:59`);
  ...
  const totalDuration = completedRecords.reduce((sum, record) => {
    ...
    const end = new Date(record.endAt ?? record.expectedEndAt).getTime();
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      <PageHeader
        eyebrow="院区管理 > 病床管理"
        title="病床管理"
        description="统一管理病床位置、状态、使用登记、使用记录和地图联动定位。"
        badge="病床台账"
        actions={
          <Button onClick={openCreateBed}>
            <Plus className="h-4 w-4" />
            新增病床
          </Button>
        }
      ...
      >
        <Field label="病床编号">
          <Input value={filters.code} placeholder="BED-302-01" onChange={(event) => setFilters((current) => ({ ...current, code: event.target.value }))} />
        </Field>
        <Field label="病床名称">
          <Input value={filters.name} placeholder="302-1床" onChange={(event) => setFilters((current) => ({ ...current, name: event.target.value }))} />
        </Field>
        <Field label="病房名称">
          <Input value={filters.wardName} placeholder="骨科三病区302" onChange={(event) => setFilters((current) => ({ ...current, wardName: event.target.value }))} />
        </Field>
        <Field label="病床状态">
          <select className="native-select" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))}>
            <option value="全部">全部</option>
            {bedStatuses.map((status) => <option key={status} value={status}>{status}</option>)}
          </select>
        </Field>
        <Field label="使用状态">
          <select className="native-select" value={filters.usageStatus} onChange={(event) => setFilters((current) => ({ ...current, usageStatus: event.target.value }))}>
            <option value="全部">全部</option>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-border/60">
          <CardTitle>病床列表</CardTitle>
          <p className="text-sm text-muted-foreground">共 {filteredBeds.length} 张病床</p>
        </CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>病床编号</TableHead>
                <TableHead>病床名称</TableHead>
                <TableHead>所属病房</TableHead>
                <TableHead>病床类型</TableHead>
                <TableHead>病床状态</TableHead>
                <TableHead>使用状态</TableHead>
                <TableHead>当前患者</TableHead>
                <TableHead>开始使用时间</TableHead>
                <TableHead>预计结束时间</TableHead>
                <TableHead>最近结束使用时间</TableHead>
                <TableHead>地图绑定状态</TableHead>
                <TableHead>更新时间</TableHead>
                <TableHead>操作</TableHead>
              </TableRow>
            </TableHeader>
              ...
              {filteredBeds.map((bed) => {
                ...
                return (
                  <TableRow
                    key={bed.id}
                    className="cursor-pointer"
                    data-state={selectedBedId === bed.id ? "selected" : undefined}
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        <Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); setSelectedBedId(bed.id); }}>
                          <BedDouble className="h-4 w-4" />
                          查看
                        </Button>
                        <Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); openEditBed(bed); }}>
                          <Edit3 className="h-4 w-4" />
                          编辑
                        </Button>
                        <Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); locateBedOnMap(bed); }}>
                          <LocateFixed className="h-4 w-4" />
                          定位
                        </Button>
                        <Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); setRecordsBed(bed); }}>
                          <ClipboardList className="h-4 w-4" />
                          使用记录
                        </Button>
                        <Button variant="ghost" size="sm" className="text-rose-600" onClick={(event) => { event.stopPropagation(); setDeleteBed(bed); }}>
                          <Trash2 className="h-4 w-4" />
                          删除
                        </Button>
                      </div>
          ...
          {selectedBed ? (
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-3">
                {[
                  ["病床编号", selectedBed.code],
                  ["病床名称", selectedBed.name],
                  ["所属病房", selectedBed.wardName],
                  ["病床类型", selectedBed.type],
                  ["病床状态", selectedBed.status],
                  ["使用状态", getBedDisplayStatus(selectedBed, data.usageRecords)]
                ].map(([label, value]) => (
                  <div key={label} className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                    <p className="text-xs text-muted-foreground">{label}</p>
                    <p className="mt-1 font-medium text-surface-900">{value}</p>
                  </div>
                ))}
              </div>
              <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3 text-sm leading-7 text-muted-foreground">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => locateBedOnMap(selectedBed)} disabled={!selectedBed.mapId}>
                  <LocateFixed className="h-4 w-4" />
                  查看地图位置
                </Button>
                ...
                <Button variant="outline" onClick={() => openEditBed(selectedBed)}>
                  <Edit3 className="h-4 w-4" />
                  重新绑定位置
                </Button>
              </div>
        ...
        <SectionCard title="病床使用管理">
          {selectedBed ? (
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">当前患者</p>
                  <p className="mt-1 font-medium">{selectedUsage?.patientName ?? "暂无"}</p>
                </div>
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">当前状态</p>
                  <p className="mt-1 font-medium">{getBedDisplayStatus(selectedBed, data.usageRecords)}</p>
                ...
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">开始使用时间</p>
                  <p className="mt-1 font-medium">{selectedUsage ? formatDateTime(selectedUsage.startAt) : "暂无"}</p>
                </div>
                <div className="rounded-xl border border-border/70 bg-surface-50 px-4 py-3">
                  <p className="text-xs text-muted-foreground">预计结束时间</p>
                  <p className="mt-1 font-medium">{selectedUsage ? formatDateTime(selectedUsage.expectedEndAt) : "暂无"}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button onClick={() => openUsageDialog(selectedBed)} disabled={selectedBed.status !== "启用"}>
                  <Plus className="h-4 w-4" />
                  使用登记
                </Button>
                <Button
                  variant="outline"
                  disabled={!getCurrentUsage(selectedBed, data.usageRecords)}
                  onClick={() => {
                    setEndingBed(selectedBed);
                    setEndDraft({ endAt: toDateTimeInput(new Date().toISOString()), note: "" });
                  }}
                >
                  结束使用
                </Button>
      ...
      <DialogFormShell
        ...
        title={editingBed ? "编辑病床" : "新增病床"}
        description="病床编号全院唯一，可直接绑定病房地图点位。"
        onSubmit={submitBed}
        submitLabel="保存"
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="病床编号" required>
            <Input value={bedDraft.code} onChange={(event) => setBedDraft((current) => ({ ...current, code: event.target.value }))} />
          </Field>
          <Field label="病床名称" required>
            <Input value={bedDraft.name} onChange={(event) => setBedDraft((current) => ({ ...current, name: event.target.value }))} />
          </Field>
          <Field label="所属病房" required>
            <Input value={bedDraft.wardName} onChange={(event) => setBedDraft((current) => ({ ...current, wardName: event.target.value }))} />
          </Field>
          <Field label="病床类型">
            <Input value={bedDraft.type} onChange={(event) => setBedDraft((current) => ({ ...current, type: event.target.value }))} />
          </Field>
          <Field label="病床状态">
            <select className="native-select" value={bedDraft.status} onChange={(event) => setBedDraft((current) => ({ ...current, status: event.target.value as BedStatus }))}>
              {bedStatuses.map((status) => <option key={status} value={status}>{status}</option>)}
            </select>
          </Field>
          <Field label="绑定地图点位">
            <select className="native-select" value={bedDraft.mapId} onChange={(event) => setBedDraft((current) => ({ ...current, mapId: event.target.value }))}>
              <option value="">暂不绑定</option>
              {data.maps.filter((map) => map.type === "病房地图").map((map) => <option key={map.id} value={map.id}>{map.name}</option>)}
            </select>
          </Field>
          <Field label="地图位置 X / Y">
            <div className="grid grid-cols-2 gap-2">
              <Input value={bedDraft.x} onChange={(event) => setBedDraft((current) => ({ ...current, x: event.target.value }))} />
              <Input value={bedDraft.y} onChange={(event) => setBedDraft((current) => ({ ...current, y: event.target.value }))} />
            </div>
          </Field>
          <div className="md:col-span-2">
            <Field label="备注">
              <Textarea value={bedDraft.note} onChange={(event) => setBedDraft((current) => ({ ...current, note: event.target.value }))} />
            </Field>
          </div>
        </div>
      ...
      <DialogFormShell
        ...
        title={`确定删除“${deleteBed?.code ?? ""}”吗？`}
        description="存在历史使用记录的病床不允许物理删除，将自动转为停用。"
        onSubmit={confirmDeleteBed}
        submitLabel="确认"
      />
      ...
      <DialogFormShell
        ...
        title="使用登记"
        description="登记后会按时间规则自动显示为使用中或已预约。"
        onSubmit={submitUsage}
        submitLabel="登记"
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="患者姓名" required>
            <Input value={usageDraft.patientName} onChange={(event) => setUsageDraft((current) => ({ ...current, patientName: event.target.value }))} />
          </Field>
          <Field label="开始使用时间">
            <Input type="datetime-local" value={usageDraft.startAt} onChange={(event) => setUsageDraft((current) => ({ ...current, startAt: event.target.value }))} />
          </Field>
          <Field label="预计结束时间">
            <Input type="datetime-local" value={usageDraft.expectedEndAt} onChange={(event) => setUsageDraft((current) => ({ ...current, expectedEndAt: event.target.value }))} />
          </Field>
          <div className="md:col-span-2">
            <Field label="备注">
              <Textarea value={usageDraft.note} onChange={(event) => setUsageDraft((current) => ({ ...current, note: event.target.value }))} />
            </Field>
          </div>
        </div>
      </DialogFormShell>
      ...
      <DialogFormShell
        open={Boolean(endingBed)}
        onOpenChange={(open) => !open && setEndingBed(null)}
        title="结束使用"
        description="结束后病床会按自动判断规则恢复为空闲或已预约。"
        onSubmit={submitEndUsage}
        submitLabel="结束使用"
      >
        <Field label="实际结束时间">
          <Input type="datetime-local" value={endDraft.endAt} onChange={(event) => setEndDraft((current) => ({ ...current, endAt: event.target.value }))} />
        </Field>
        <Field label="备注">
          <Textarea value={endDraft.note} onChange={(event) => setEndDraft((current) => ({ ...current, note: event.target.value }))} />
        </Field>
      </DialogFormShell>
      ...
      <Dialog open={Boolean(recordsBed)} onOpenChange={(open) => !open && setRecordsBed(null)}>
        <DialogContent className="w-[min(96vw,980px)] max-w-none">
          <DialogHeader>
            <DialogTitle>{recordsBed?.code ?? ""} 使用记录</DialogTitle>
            <DialogDescription>保存全部历史记录，并自动统计使用次数、累计使用时长和平均使用时长。</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
                ...
                <TableBody>
                  {selectedRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>{formatDateTime(record.startAt)}</TableCell>
                      <TableCell>{record.endAt ? formatDateTime(record.endAt) : "未结束"}</TableCell>
                      <TableCell>{formatDuration(record.startAt, record.endAt ?? record.expectedEndAt)}</TableCell>
                      <TableCell>{record.registrar}</TableCell>
                      <TableCell>{formatDateTime(record.registeredAt)}</TableCell>
                      <TableCell>{record.endNote ?? record.note}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
